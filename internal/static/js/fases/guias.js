...
export async function cargarGuias(apId) {
    ...
    const tieneGuias = state.guiasCache.length > 0;
    document.getElementById('guias-lock').style.display         = tieneGuias ? 'none'  : 'block';
    document.getElementById('actividades-section').style.display = tieneGuias ? 'block' : 'none';

    // Si la AP no tenía guías y ahora tiene, recargar actividades
    if (tieneGuias && state._apData[state.selectedActProyId]?.guias_count === 0) {
        state._apData[state.selectedActProyId].guias_count = state.guiasCache.length;
        document.dispatchEvent(new CustomEvent('fases:actividades-refresh'));
    }
}
